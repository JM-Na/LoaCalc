...
public class MarketProcessor {
    ...
    private final HoneIngredientRepository ingredientRepository;

    public void initPrice() {

        initHoneIngredientsPrice();
        initEngravingBookPrice();
    }

    //    private void initHoneIngredientsPrice() {
...
//
//                HoneIngredientsData.setIconAndPrice(name, icon, yDayAvgPrice);
//            }
//        }
//    }
    public void initHoneIngredientsPrice() {
        RequestMarketItems requestMarketItems3 = createIngredientsRequest(3);
        RequestMarketItems requestMarketItems4 = createIngredientsRequest(4);

        List<Item> items = marketClient.getMarketItems(requestMarketItems3).getItems();
        items.addAll(marketClient.getMarketItems(requestMarketItems4).getItems());
        ...
        for (Item item : items) {
            ...
            ingredientRepository.save(new HoneIngredient(name, icon, yDayAvgPrice));
        }
    }

    private static RequestMarketItems createIngredientsRequest(int tier) {
        RequestMarketItems requestMarketItems = new RequestMarketItems();
        requestMarketItems.setSort("GRADE");
        requestMarketItems.setCategoryCode(50000);
        requestMarketItems.setCharacterClass(null);
        requestMarketItems.setItemTier(tier);
        requestMarketItems.setItemGrade(null);
        requestMarketItems.setSortCondition("ASC");
    ...
    public void initEngravingBookPrice() {
        for (int pageNo = 1; pageNo <= 2; pageNo++) {
            ...
            for (Item item : items) {
                ...
                engravingBookRepository.save(new RelicEngravingBook(name, icon, yDayAvgPrice));
            }
        }
    }
